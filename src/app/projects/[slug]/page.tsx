...
function SectionIntro({ eyebrow, title, children }: { eyebrow: string; title: string; children: React.ReactNode }) {
  return (
    <div className="grid gap-5 md:grid-cols-12">
      <div className="md:col-span-4"><p className="font-mono text-xs font-semibold uppercase tracking-[0.16em] text-indigo-700">{eyebrow}</p><h2 className="mt-3 text-[clamp(2rem,8vw,2.5rem)] font-semibold leading-[1.05] tracking-[-0.04em]">{title}</h2></div>
  ...
  return (
    <main id="main-content" className="pb-20 pt-12 sm:pb-28 sm:pt-20">
      <Container>
        ...
        <header className="technical-grid-light mt-10 w-full min-w-0 max-w-full overflow-hidden rounded-2xl bg-surface p-6 sm:mt-14 sm:p-9">
        ...
        <div className="mt-12 sm:mt-16"><OrderAutomationPreview wide /></div>

        <section className="mt-16 grid gap-8 rounded-2xl bg-surface p-6 sm:mt-24 sm:p-8 md:grid-cols-12">
          <h2 className="text-xs font-bold uppercase tracking-[0.16em] text-indigo-700 md:col-span-4">Overview</h2>
          <div className="md:col-span-8"><p className="max-w-3xl text-xl leading-9">{project.fullDescription}</p><dl className="mt-10 grid gap-3 sm:grid-cols-3">{[["Type", project.category], ["Context", project.context], ["Role", project.role]].map(([label, value]) => <div key={label} className="rounded-lg bg-background p-5"><dt className="font-mono text-[0.66rem] font-semibold uppercase tracking-[0.14em] text-black/45">{label}</dt><dd className="mt-3 font-semibold">{value}</dd></div>)}</dl></div>
        </section>

        <section className="mt-20 rounded-2xl bg-surface p-6 sm:mt-28 sm:p-8"><SectionIntro eyebrow="01 / The problem" title="Reliable data was being maintained by hand."><p>{project.problem}</p><p className="mt-5">Different labels could describe the same hardware, while repeated records and out-of-order dates made the workbook harder to maintain. The challenge was operational consistency, not technical novelty.</p></SectionIntro></section>
        ...
        <section className="technical-grid mt-20 overflow-hidden rounded-2xl bg-black p-6 text-background sm:mt-28 sm:p-10"><div className="grid gap-8 md:grid-cols-12"><p className="font-mono text-xs font-semibold uppercase tracking-[0.16em] text-indigo-700 md:col-span-4">Outcome / Complete</p><p className="text-[clamp(1.75rem,6vw,2.75rem)] font-semibold leading-[1.15] tracking-[-0.04em] text-background/90 md:col-span-8">{project.result}</p></div></section>

        <section className="mt-20 grid gap-12 sm:mt-28 md:grid-cols-12">
          <div className="rounded-xl bg-surface p-6 md:col-span-4"><p className="font-mono text-xs font-semibold uppercase tracking-[0.16em] text-indigo-700">Technologies</p><ul className="mt-5 grid gap-2">{project.technologies.map((technology) => <li key={technology} className="font-medium text-black/70">{technology}</li>)}</ul></div>
